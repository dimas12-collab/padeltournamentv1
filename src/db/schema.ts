...
export const matches = pgTable("matches", { id: text("id").primaryKey(), eventId: text("event_id").notNull().references(() => events.id), categoryId: text("category_id").notNull().references(() => categories.id), groupId: text("group_id").references(() => groups.id), stage: stageEnum("stage").notNull(), lineupCategory: text("lineup_category"), round: integer("round"), teamAId: text("team_a_id").references(() => teams.id), teamBId: text("team_b_id").references(() => teams.id), courtId: text("court_id").references(() => courts.id), scheduledAt: timestamp("scheduled_at", { withTimezone: true }), status: matchStatusEnum("status").notNull(), winnerId: text("winner_id").references(() => teams.id), nextMatchId: text("next_match_id"), nextMatchSlot: text("next_match_slot"), notes: text("notes"), deletedAt: timestamp("deleted_at", { withTimezone: true }), ...timestamps }, (table) => [index("matches_event_schedule_idx").on(table.eventId, table.scheduledAt), index("matches_group_status_idx").on(table.groupId, table.status)]);
