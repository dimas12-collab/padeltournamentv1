...
export const RANKING_CRITERIA = ['PTS','H2H','SD','GD','GF','W','L','MP'] as const;
...
export interface Rules { win:number; loss:number; split:boolean; win20:number; win21:number; loss12:number; loss02:number; tieBreak:RankingCriterion[]; qualifiersPerGroup?:number; bracketSize?:2|4|8; standingColumns?:StandingColumn[]; }
...
export interface Match extends Base { eventId:string; categoryId:string; groupId?:string; stage:Stage; lineupCategory?:LineupCategory; teamAId:string; teamBId:string; courtId?:string; scheduledAt?:string; status:MatchStatus; sets:MatchSet[]; winnerId?:string; notes?:string; nextMatchId?:string; nextSlot?:'A'|'B'; }
