...
export async function readTournamentData(includePrivate = false): Promise<Data> {
  ...
  return {
    ...
    matches: matchRows.map((row) => ({ id: row.id, name: `${teamById.get(row.teamAId ?? "")?.name ?? row.stage.replaceAll("_", " ")} vs ${teamById.get(row.teamBId ?? "")?.name ?? "TBD"}${row.lineupCategory ? ` · ${row.lineupCategory}` : ""}`, eventId: row.eventId, categoryId: row.categoryId, groupId: row.groupId ?? undefined, stage: row.stage, lineupCategory: row.lineupCategory ? row.lineupCategory as Match["lineupCategory"] : undefined, teamAId: row.teamAId ?? "", teamBId: row.teamBId ?? "", courtId: row.courtId ?? undefined, scheduledAt: iso(row.scheduledAt), status: row.status, sets: (setsByMatch.get(row.id) ?? []).sort((a, b) => a.setNumber - b.setNumber).map(({ a, b }) => ({ a, b })), winnerId: row.winnerId ?? undefined, notes: row.notes ?? undefined, nextMatchId: row.nextMatchId ?? undefined, nextSlot: row.nextMatchSlot === "A" || row.nextMatchSlot === "B" ? row.nextMatchSlot : undefined, deletedAt: iso(row.deletedAt), createdAt: iso(row.createdAt) })),
...
async function replaceMatch(item: Match) {
  await db.transaction(async (tx) => {
    await tx.insert(matches).values({ id: item.id, eventId: item.eventId, categoryId: item.categoryId, groupId: nullable(item.groupId), stage: item.stage, lineupCategory: nullable(item.lineupCategory), teamAId: nullable(item.teamAId), teamBId: nullable(item.teamBId), courtId: nullable(item.courtId), scheduledAt: item.scheduledAt ? new Date(item.scheduledAt) : null, status: item.status, winnerId: nullable(item.winnerId), nextMatchId: nullable(item.nextMatchId), nextMatchSlot: nullable(item.nextSlot), notes: nullable(item.notes), deletedAt: item.deletedAt ? new Date(item.deletedAt) : null, updatedAt: new Date() }).onConflictDoUpdate({ target: matches.id, set: { categoryId: item.categoryId, groupId: nullable(item.groupId), stage: item.stage, lineupCategory: nullable(item.lineupCategory), teamAId: nullable(item.teamAId), teamBId: nullable(item.teamBId), courtId: nullable(item.courtId), scheduledAt: item.scheduledAt ? new Date(item.scheduledAt) : null, status: item.status, winnerId: nullable(item.winnerId), nextMatchId: nullable(item.nextMatchId), nextMatchSlot: nullable(item.nextSlot), notes: nullable(item.notes), deletedAt: item.deletedAt ? new Date(item.deletedAt) : null, updatedAt: new Date() } });
